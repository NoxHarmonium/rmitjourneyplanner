...
namespace RmitJourneyPlanner.CoreLibraries.RoutePlanners.Evolutionary.RouteGenerators
{
    ...
    /// </summary>
    public class AlRouteGenerator : IRouteGenerator
    {
        ...
        public AlRouteGenerator(EvolutionaryProperties properties)
        {
...
Regenerate:
            ...
            bool findCentral = false; //this.random.NextDouble() <= 0.3;
            bool foundCentral = false;

            while (this.destinationNodes[0].Id != current.Id)
            {
                // Advance pointer
                current = next;
                ...
                // routeDict.Add(current, routeIndex++);
                if (current.Id != 0)
                {
...

                {
                    ...
                    var euclidianDistances = new Dictionary<int, double>();

                    // Logger.Log(this,"-->-->-->Adjacent nodes:");
                    var adjacent = provider.GetAdjacentNodes(current);

                    foreach (MetlinkNode node in adjacent)
                    {
                        if (node.Id != current.Id)
                        {
                            ...
                            */
                            ...
                            if (findCentral && !foundCentral)
                            {
                                var flindersStreet = (MetlinkNode)provider.GetNodeFromId(19973);
                                euclidianDistance = GeometryHelper.GetStraightLineDistance(
                                    location, new Location(flindersStreet.Latitude, flindersStreet.Longitude));
                            }
                            else
                            {
                                euclidianDistance = GeometryHelper.GetStraightLineDistance(
                            ...
                            euclidianDistances[node.Id] = euclidianDistance;
                    ...
                    if (p < properties.ProbMinDistance)
                    {
                        p = this.random.NextDouble();

                        bool transfer = p < properties.ProbMinTransfers;

                        int minNode = -1;

                        double minDist = double.MaxValue;
                        foreach (var kvp in euclidianDistances)
                        {
                            bool isTransfer = !provider.RoutesIntersect(provider.GetNodeFromId(kvp.Key), current);
                        ...
                        if (minNode != -1)
                        {
                            next = (MetlinkNode)provider.GetNodeFromId(minNode);
                        }
                    }
                    else
                    {
                        while (adjacent.Count > 1 && next == null)
                        {
                            int index = this.random.Next(adjacent.Count - 1);
                            var nextMetlinkNode = (MetlinkNode)adjacent.ElementAt(index + 1);
                            if (routeDict.ContainsKey(nextMetlinkNode))
                            {
                                adjacent.Remove(nextMetlinkNode);
                            }
                            else
                            {
                                next = nextMetlinkNode;
                            }
                        }
                    }
                    ...
                    if (next == null)
                    {
                        MetlinkNode nextNode = null;
                        
                        for (int i = 1; i < adjacent.Count; i++)
                        {
                            nextNode = (MetlinkNode)adjacent[i];
                            if (!routeDict.ContainsKey(nextNode))
                            {
                                nextNode = (MetlinkNode)adjacent[i++];
                        ...
                        next = nextNode;//?? this.destinationNodes[0];
                    }
