...
namespace RmitJourneyPlanner.CoreLibraries.DataProviders.Metlink
{
    ...
    /// </summary>
    public class AltMetlinkDataProvider : INetworkDataProvider, IDisposable
    {
            ...
            INetworkNode source, INetworkNode destination, DateTime departureTime, int routeId)
        {
            ...
            object[] result = this.database.GetFastData(query);

            // departureTime.
            if (result.Length > 0)
            {
                DateTime departTime = this.ParseDate(result[1].ToString());
                DateTime arrivalTime = this.ParseDate(result[2].ToString());
